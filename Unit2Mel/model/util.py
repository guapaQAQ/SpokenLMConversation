...
def sampling_given_noise_schedule(h, eval_n_sch, net, km, ref):
    # km, ref, audio = batch
    ...
    for n in range(N-1, -1, -1):
        ...
        x_t -= beta_infer[n]/torch.sqrt(1-alpha_infer[n]**2.)*eps
        x_t /= torch.sqrt(1-beta_infer[n])
        if n > 0:
            x_t = x_t+sigma_infer[n]*std_normal(x_t.shape, km.device)
    x = x_t*h.mel_s+h.mel_m

    return x


def theta_timestep_loss(h, dh, net, batch):
    km, ref, audio = batch
    ...
    ref = mel_fn(ref, h.n_fft, h.num_mels, h.sampling_rate,
                 h.hop_size, h.win_size, h.fmin, h.fmax)
    ref = (ref-h.mel_m)/h.mel_s
    mel = mel_fn(audio, h.n_fft, h.num_mels, h.sampling_rate,
                 h.hop_size, h.win_size, h.fmin, h.fmax)
    mel = (mel-h.mel_m)/h.mel_s

    # prepare t, coef, z, x_t
    ts = torch.randint(T, size=(mel.size(0), 1, 1)).to(mel.device)
    alpha = torch.cat([torch.Tensor([1]).to(alpha.device), alpha])
    z = std_normal(mel.shape, mel.device)
    alpha_cur = alpha[ts]
    delta = (1-alpha[ts]**2.).sqrt()
    x_t = alpha_cur*mel+delta*z

    # precompute, main net, loss
    c, ref = net.URE(km, ref)
    eps = net(x_t, ts.squeeze(1), c, ref)
    loss = F.mse_loss(eps, z)
